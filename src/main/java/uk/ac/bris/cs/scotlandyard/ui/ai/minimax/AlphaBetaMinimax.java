...
public class AlphaBetaMinimax implements GenericMiniMax {
    ...
    private final MinimaxCache<MinimaxState> cache;
    ...
    public GenericMiniMax.MinimaxResult minimaxRoot(boolean isMrX, GameState root, int depth, int mrXLocation, boolean allowDoubleMoves) {
     ...
     */
    public GenericMiniMax.MinimaxResult minimax(boolean isMrX,
        ...
        if (depth <= 0 || availableMoves.isEmpty()) {
            ...
            return new GenericMiniMax.MinimaxResult(head, moveScorer.calculateScore(head.commencedBy().isMrX(), alternativeMoves, head, node));
        }


        MinimaxState key = new MinimaxState(alpha, beta, depth, node.getAvailableMoves(), isMrX);
    ...
    private FullMinimaxResult minimise(GameState node, int depth, int mrXLocation, double alpha, double beta, @Nullable Move head, boolean allowDoubleMoves, Collection<Move> availableMoves) {
        GenericMiniMax.MinimaxResult value = null;
    ...
    record FullMinimaxResult(GenericMiniMax.MinimaxResult res, double alpha, double beta, double depth) {
